...
public class GridEvent extends VerticalLayout {
	...
	public GridEvent(Advice advice, String userName, String who) {
		username = userName;
		tipo = who;
		// Recupero annunci
		try {
		...
		for (int i = 0; i < elenco.size(); i++) {
			...
			if(tipo.equals("mine")) {
				...
				Label utente = new Label("Iscritto all' annuncio di <h5>" + elenco.get(i).getUsername() + "</h5>", Label.CONTENT_XHTML);
				...
				Label utente = new Label("<h5>" + username
						+ "</h5> ha risposto al tuo annuncio", Label.CONTENT_XHTML);
				utente.addStyleName(Reindeer.PANEL_LIGHT);
				layout.addComponent(utente);
				...
				Label utente = new Label("<h5>" + username
						+ "</h5> richiede di eliminare evento", Label.CONTENT_XHTML);
				utente.addStyleName(Reindeer.PANEL_LIGHT);
				layout.addComponent(utente);
